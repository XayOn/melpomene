...
@click.option('--base_path', help='path to store images')
@click.option('--ngrok_url', help='public ngrok url to base path')
@click.option('--end', help='stop after <end> results', type=int)
@click.option('--step', help='parse each <step> frames', type=int)
def print_results(video, base_path, ngrok_url, end, step):
    ...
    This requires to have a base_path where all extracted frames will be stored, that is actually accessible trough a external url.
    ...
    """
    result = islice(execute_search(video, base_path, ngrok_url, step), end)
